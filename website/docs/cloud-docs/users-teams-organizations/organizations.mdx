...
Organization owners can also control whether [workspace administrators](/cloud-docs/users-teams-organizations/permissions#workspace-admins) may delete workspaces which are currently managing resources. Deleteing a workspace with [resources under management](/cloud-docs/workspaces/settings#deleting-a-workspace-with-resources-under-management) will result in any infrastructure provisioned by that workspace being left unmanaged by Terraform.
...
You can enforce health assessments for all eligible workspaces or let each workspace opt in to health assessments through workspace settings. Enforcing health assessments overrides workspace settings but does not change them. Refer to [Health Assessments](/cloud-docs/workspaces/settings/health-assessments) in the workspaces documentation for more details.
